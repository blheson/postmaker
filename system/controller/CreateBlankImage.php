...
// use ImageDimension;
    ...
    public function check_image($data,$link){
        ...
        // $upload_path = $root_dir.$link['long'];
         ...
         if ($mime_type != 'image') {
          $_SESSION['error'] = 'The file must be an image';return 'The file must be an image';
        }
        if (!in_array($file_ext, $allowed)) {
          $_SESSION['error'] = 'The file extension must be a png, jpg, jpeg, gif';return 'The file extension must be a png, jpg, jpeg, gif';
        }
        if ($file_size > 10000000) {
          $_SESSION['error'] = 'The file size must be under 10MB';return 'The file size must be under 15MB';
        }
        if ($file_ext != $mime_ext && ($mime_ext =='jpeg' && $file_ext != 'jpg' )) {
          $_SESSION['error'] = 'The file extension does not match the original file';return 'The file extension does not match the original file';
        }	
